...
if(segmento == "Ejecutivo de Negocios"):
    ...
    with form:
        ...
        enviar = form.form_submit_button("Calcular Incentivos", on_click = Calculo_CC, args = (ag, filas))

        print(ag)
    if enviar:
        a, b, c = Calculo_CC(ag, filas)
        st.title("Total a Incentivar: Q "+ str(min(6500.00, round(c,2))))
