...
if(segmento == "Ejecutivo de Negocios"):
    ...
    with form:
        ...
        if enviar:
            ...
            st.title(Calculo_EN(meses, md_t, d_n, md_n, p1_i, p1_o, p1_dif, p30_i, p30_o, p30_dif))
    ...
    with form:
        ...
        if enviar:
            # st.write(Calculo_EN(meses, md_t, d_n, md_n, p1_i, p1_o, p1_dif, p30_i, p30_o, p30_dif))
            st.title(Calculo_OBC(ci, cf, dt, dn, md_i, p1_i, p1_o, p1_dif, p30_i, p30_o, p30_dif))

elif(segmento == "Operador de CC"):

    form = st.form("CC")
    filas = {}
    datos = {0: 'Agencia', 1: 'Meta', 2: 'Recuperado', 3: 'Meta Clientes', 4: 'Clientes Recup.'}
    j = 0
    ...
    with st.expander("Numero de Agencias", expanded=True):
        ...
        for ix in range(ag):
            for x, jx in enumerate(cols):
                filas["f_%i_%i" %(ix, j)] = jx.number_input("%s %i" %(datos[j], ix), min_value=0, format="%i", step = 1)
                j = j + 1
    ...
    enviar = st.button("Calcular Incentivos", on_click = Calculo_CC, args = (ag, filas))


        # enviar = st.form_submit_button("Calcular Incentivos", on_click = Calculo_EN, args = (meses, md_t, d_n, md_n, p1_i, p1_o, p1_dif, p30_i, p30_o, p30_dif))
        #enviar = form.form_submit_button("Calcular Incentivos", on_click=Calculo_CC, args=(ci, cf, ai, af, mr_i, er_o))


        #if enviar:
            # st.write(Calculo_EN(meses, md_t, d_n, md_n, p1_i, p1_o, p1_dif, p30_i, p30_o, p30_dif))
            #st.title(Calculo_CC(ci, cf, ai, af, mr_i, er_o))
